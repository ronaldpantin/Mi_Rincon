...
// Sistema de Reservas Ultra Simplificado - CORREGIDO
// ========================================
...
const STATUS_COLUMN = 16 // Columna P (Estado) - índice 16 (base 1)
const EMAIL_COLUMN = 3 // Columna C (Email) - índice 3 (base 1)
const RESERVATION_NUMBER_COLUMN = 17 // Columna Q (Número Reserva) - índice 17 (base 1)
...
function doPost(e) {
  try {
    ...
    const selectedAreasText =
      (reservationDetails.selectedAreasDetails || []).map((area) => `${area.name} ($${area.price})`).join(", ") ||
      "Ninguna"

    // Preparar datos para insertar (con categorías de personas y áreas)
    const rowData = [
      ...
      reservationDetails.entriesSubtotalUSD || 0, // K: Subtotal Entradas USD
      reservationDetails.areasSubtotalUSD || 0, // L: Subtotal Áreas USD
      reservationDetails.totalUSD, // M: Total USD
      reservationDetails.finalTotalVEF, // N: Total VEF
      transactionReference, // O: Referencia Pago
      "PENDIENTE", // P: Estado
      "", // Q: Número Reserva
      selectedAreasText, // R: Áreas Seleccionadas
      `ID: ${solicitudId}`, // S: Notas
    ]
...
function onEdit(e) {
  try {
    ...
    // Verificar si es la columna de Estado (P = columna 16)
    ...
    // Obtener datos de la fila (ahora con más columnas)
    ...
    const reservationData = {
      ...
      entradasSubtotalUSD: rowData[10],
      areasSubtotalUSD: rowData[11],
      totalUSD: rowData[12],
      totalVEF: rowData[13],
      referenciaPago: rowData[14],
      estado: rowData[15],
      numeroReserva: rowData[16],
      areasSeleccionadas: rowData[17],
      notas: rowData[18],
    }
...
function enviarEmailConfirmacionCliente(reservationDetails, transactionReference, solicitudId) {
  try {
    ...
    // Preparar texto de áreas seleccionadas
    const areasText =
      (reservationDetails.selectedAreasDetails || []).length > 0
        ? reservationDetails.selectedAreasDetails.map((area) => `${area.name} ($${area.price} USD)`).join(", ")
        : "Ninguna área adicional seleccionada"
        ...
        <div style="padding: 30px; background: #f9fafb;">
          ...
          <div style="background: white; padding: 25px; border-radius: 12px; margin-bottom: 25px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Entradas:</td><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb;">$${reservationDetails.entriesSubtotalUSD || 0} USD</td></tr>
              <tr><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Áreas Adicionales:</td><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb;">$${reservationDetails.areasSubtotalUSD || 0} USD</td></tr>
              ...
              <tr><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Áreas Seleccionadas:</td><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb;">${areasText}</td></tr>
...
ENTRADAS: $${reservationData.entradasSubtotalUSD}
ÁREAS: $${reservationData.areasSubtotalUSD}
TOTAL: $${reservationData.totalUSD} USD
ÁREAS: ${reservationData.areasSeleccionadas}
        ...
        <div style="padding: 30px; background: #f9fafb;">
          ...
          <div style="background: white; padding: 25px; border-radius: 12px; margin-bottom: 25px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              ...
              <tr><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Entradas:</td><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb;">$${reservationData.entradasSubtotalUSD} USD</td></tr>
              <tr><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Áreas Adicionales:</td><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb;">$${reservationData.areasSubtotalUSD} USD</td></tr>
              ...
              <tr><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Áreas Seleccionadas:</td><td style="padding: 12px 0; border-bottom: 1px solid #e5e7eb;">${reservationData.areasSeleccionadas}</td></tr>
...
function testFunction() {
  ...
  sheet.appendRow([
    ...
    15, // entradas subtotal USD
    25, // áreas subtotal USD
    40, // total USD
    1460, // total VEF
    "TEST123456",
    "PENDIENTE",
...
function testEmailAutorizacion() {
  ...
  const reservationData = {
    ...
    exonerados: 1,
    totalPersonas: 4,
    entradasSubtotalUSD: 15,
    areasSubtotalUSD: 25,
    totalUSD: 40,
    totalVEF: 1460,
    referenciaPago: "TEST123456",
    areasSeleccionadas: "Caney Principal ($50)",
  }

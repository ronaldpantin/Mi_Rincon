...
// Sistema de Reservas con Fecha de Visita - VERSIÓN ACTUALIZADA
// ========================================
...
const STATUS_COLUMN = 11 // Columna K (Estado) - índice 11 (base 1)
const EMAIL_COLUMN = 4 // Columna D (Email) - índice 4 (base 1)
const RESERVATION_NUMBER_COLUMN = 12 // Columna L (Número Reserva) - índice 12 (base 1)
...
function doPost(e) {
  try {
    ...
    // Preparar datos para insertar (con fecha de visita)
    const rowData = [
      new Date(), // A: FechaHora Solicitud
      formattedVisitDate, // B: Fecha de Visita
      `${reservationDetails.bookerFirstName} ${reservationDetails.bookerLastName}`, // C: Nombre
      reservationDetails.bookerEmail, // D: Email
      reservationDetails.bookerPhone, // E: Teléfono
      reservationDetails.totalUSD, // F: Total USD
      reservationDetails.finalTotalVEF, // G: Total VEF
      transactionReference, // H: Referencia Pago
      "ENTRADA GENERAL", // I: Tipo
      "PROCESO SIMPLIFICADO", // J: Comprobante
      "PENDIENTE", // K: Estado
      "", // L: Número Reserva
      `ID: ${solicitudId}`, // M: Notas
    ]
...
function onEdit(e) {
  try {
    ...
    // Verificar si es la columna de Estado (K = columna 11)
    ...
    const reservationData = {
      fechaHoraSolicitud: rowData[0],
      fechaVisita: rowData[1],
      nombreCompleto: rowData[2],
      email: rowData[3],
      telefono: rowData[4],
      totalUSD: rowData[5],
      totalVEF: rowData[6],
      referenciaPago: rowData[7],
      tipo: rowData[8],
      comprobante: rowData[9],
      estado: rowData[10],
      numeroReserva: rowData[11],
      notas: rowData[12],
    }
...
function testFunction() {
  console.log("=== PRUEBA GENERAL DEL SISTEMA CON FECHA ===")

  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_NAME)
  sheet.appendRow([
    new Date(), // Fecha/Hora Solicitud
    "25/01/2025", // Fecha de Visita
    "Juan Pérez", // Nombre
    ...
    "ENTRADA GENERAL", // Tipo
    "PRUEBA", // Comprobante
    "PENDIENTE", // Estado
    "", // Número Reserva
    "Prueba del sistema con fecha de visita", // Notas
  ])
...
function testEmailAutorizacion() {
  console.log("=== PRUEBA EMAIL AUTORIZACIÓN CON FECHA ===")
  ...
  console.log("✅ Prueba de email de autorización con fecha completada")
}
